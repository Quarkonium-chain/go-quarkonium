...
See `WebDebugger` and `TestWebDebuggerManual` in [go-algorand sources](https://github.com/Quarkonium-chain/go-quarkonium/tree/master/data/transactions/logic) for more details.
...
Transaction(s) are JSON or MessagePack (`goal clerk` compatible) serialized instances of `transactions`. See [samples dir](https://github.com/Quarkonium-chain/go-quarkonium/tree/master/cmd/tealdbg/samples) for more examples.
